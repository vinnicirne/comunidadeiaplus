...
export default function LeftSidebar({ categories }: { categories?: any[] }) {
  ...
  const getLinkClass = (path: string, exact: boolean = false) => {
    ...
    return isActive
      ? "flex items-center px-3 py-2 rounded-lg bg-[#6366f1]/20 text-[#a5b4fc] border border-[#6366f1]/30 font-semibold text-[14px] transition-colors"
      : "flex items-center px-3 py-2 rounded-lg text-[#94a3b8] hover:bg-[#1e293b] hover:text-[#f8fafc] text-[14px] font-medium transition-colors";
  };

  return (
    <aside className="hidden lg:block w-64 shrink-0 fixed top-16 bottom-0 overflow-y-auto py-6 pr-4 pl-4 bg-[#0f172a] border-r border-[#1e293b]">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2 pt-2 border-t border-[#1e293b]">
          <div className="px-3 text-[11px] text-[#64748b] uppercase tracking-wider font-semibold">
            Categorias
          </div>
          <nav className="flex flex-col gap-1">
