              ...
              <div className="flex flex-col w-full">
                <div className="flex flex-col gap-space-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-space-lg">
                    {categories.filter(c => c.is_active).map((cat, index) => {
                      const colors = [
                        'primary',
                        'secondary',
                        'tertiary',
                      ...
                      return (
                        <div key={cat.id} className="group bg-surface-container-lowest rounded-xl p-space-lg shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between relative overflow-hidden">
                          <div className={`absolute top-0 right-0 w-32 h-32 bg-${color}/5 rounded-bl-full pointer-events-none transition-transform duration-500 group-hover:scale-110`}></div>
                          <div className="flex flex-col gap-space-md relative z-10">
                            <div className="flex items-start justify-between">
                              <div className={`w-12 h-12 rounded-xl bg-${color}-fixed flex items-center justify-center text-on-${color}-fixed shadow-sm`}>
                            ...
                            <div className="flex items-center gap-space-md py-space-xs text-outline font-label-sm text-label-sm">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-medium text-on-surface">Ativo</span>
                              </div>
                            </div>
                          </div>
                          <div className="pt-space-md mt-space-md flex items-center justify-between relative z-10">
                            ...
                            <Link aria-label={`Acessar categoria ${cat.name}`} className="absolute inset-0 z-20" href={`/categoria/${cat.slug}`}></Link>
                          </div>
                        </div>
                      )
                    })}
